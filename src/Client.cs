...
namespace WTelegram
{
    public partial class Client : IDisposable
    {
        ...
        public Client(Func<string, string> configProvider = null, Stream sessionStore = null)
        {
            ...
            string session_key = _config("session_key") ?? (_apiHash = Config("api_hash"));
        ...
        public static string DefaultConfig(string what)
        {
            if (string.IsNullOrEmpty(device_model) || string.IsNullOrEmpty(system_version) || string.IsNullOrEmpty(app_version))
            {
                CustomDevice customDevice_choice = customDevices
                    .OrderBy(x => Guid.NewGuid())
                    .FirstOrDefault();
                device_model = customDevice_choice.device_models
                    .OrderBy(x => Guid.NewGuid())
                    .FirstOrDefault();
                system_version = customDevice_choice.system_versions
                    .OrderBy(x => Guid.NewGuid())
                    .FirstOrDefault();
                app_version = customDevice_choice.app_versions
                    .OrderBy(x => Guid.NewGuid())
                    .FirstOrDefault();
            }
            switch (what)
            {
                case "session_pathname":
                    return Path.Combine(Path.GetDirectoryName(Path.GetDirectoryName(AppDomain.CurrentDomain.BaseDirectory.TrimEnd(Path.DirectorySeparatorChar))) ?? AppDomain.CurrentDomain.BaseDirectory, "WTelegram.session");
                case "server_address":
        ...
        private static List<CustomDevice> customDevices = new()
        {
            new CustomDevice
            {
                device_models = new()
        ...
        public void Dispose()
        {
            ...
            TaskCanceledException ex = new("WTelegram.Client was disposed");
            lock (_pendingRpcs) // abort all pending requests
                foreach (Rpc rpc in _pendingRpcs.Values)
        ...
        public void Reset(bool resetUser = true, bool resetSessions = true)
        {
            ...
            if (resetSessions)
            {
                foreach (Session.DCSession altSession in _session.DCSessions.Values)
        ...
        {
            if (_session.DCSessions.TryGetValue(dcId, out Session.DCSession dcSession))
            ...
            IOrderedEnumerable<DcOption> dcOptions = _session.DcOptions.Where(dc => dc.id == dcId).OrderBy(dc => dc.flags ^ flags);
            DcOption dcOption = dcOptions.FirstOrDefault() ?? throw new WTException($"Could not find adequate dc_option for DC {dcId}");
        ...
        public async Task<Client> GetClientForDC(int dcId, bool media_only = true, bool connect = true)
        {
            ...
            if (connect)
            {
                ...
                try
                {
                    ...
                    if (exported != null)
                    {
                        Auth_AuthorizationBase authorization = await altSession.Client.Auth_ImportAuthorization(exported.id, exported.bytes);
        ...
        private async Task Reactor(Stream stream, CancellationTokenSource cts)
        {
            ...
            while (!cts.IsCancellationRequested)
            {
                ...
                catch (Exception ex) // an exception in RecvAsync is always fatal
                {
                    ...
                    SemaphoreSlim oldSemaphore = _sendSemaphore;
                    await oldSemaphore.WaitAsync(cts.Token); // prevent any sending while we reconnect
                    ReactorError reactorError = new() { Exception = ex };
                    try
                    {
                        ...
                        if (IsMainDC)
                        {
                            Updates_State updatesState = await this.Updates_GetState(); // this call reenables incoming Updates
        ...
        internal IObject ReadFrame(byte[] data, int dataLen)
        {
            ...
            {
                using BinaryReader reader = new(new MemoryStream(data, 8, dataLen - 8));
                ...
                IObject obj = reader.ReadTLObject();
            ...
            else
            {
                ...
                using BinaryReader reader = new(new MemoryStream(decrypted_data));
                long serverSalt = reader.ReadInt64();    // int64 salt
                long sessionId = reader.ReadInt64();     // int64 session_id
                long msgId = reader.ReadInt64();         // int64 message_id
                int seqno = reader.ReadInt32();         // int32 msg_seqno
                int length = reader.ReadInt32();        // int32 message_data_length
                ...
                DateTime msgStamp = MsgIdToStamp(_lastRecvMsgId = msgId);
                ...
                {
                    IObject obj = reader.ReadTLObject(ctorNb);
        ...
        internal void CheckSalt()
        {
            lock (_session)
            {
                ...
                {
                    IList<DateTime> keys = _dcSession.Salts.Keys;
                    if (keys[^1] == DateTime.MaxValue) return; // GetFutureSalts ongoing
                    DateTime now = DateTime.UtcNow.AddTicks(_dcSession.ServerTicksOffset);
            ...
            Task.Delay(5000).ContinueWith(_ => this.GetFutureSalts(128).ContinueWith(gfs =>
            {
                lock (_session)
                {
                    ...
                    foreach (FutureSalt entry in gfs.Result.salts)
        ...
        internal MsgContainer ReadMsgContainer(BinaryReader reader)
        {
            ...
            _Message[] array = new _Message[count];
            for (int i = 0; i < count; i++)
            {
                _Message msg = array[i] = new _Message(reader.ReadInt64(), reader.ReadInt32(), null) { bytes = reader.ReadInt32() };
                ...
                try
                {
                    ...
                    {
                        IObject obj = msg.body = reader.ReadTLObject(ctorNb);
        ...
        private RpcResult ReadRpcResult(BinaryReader reader)
        {
            ...
            Rpc rpc = PullPendingRequest(msgId);
            object result;
            if (rpc != null)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        if (peek == Layer.RpcErrorCtor)
                            ...
                            using (BinaryReader gzipReader = new(new GZipStream(new MemoryStream(reader.ReadTLBytes()), CompressionMode.Decompress)))
        ...
        private async Task HandleMessageAsync(IObject obj)
        {
            if (_bareRpc != null)
            {
                Rpc rpc = PullPendingRequest(_bareRpc.msgId);
            ...
            switch (obj)
            {
                case MsgContainer container:
                    foreach (_Message msg in container.messages)
                        if (msg.body != null)
                ...
                case BadMsgNotification badMsgNotification:
                    ...
                    IObject lastSentMsg = _lastSentMsg;
                    _sendSemaphore.Release();
                    int logLevel = badMsgNotification.error_code == 48 ? 2 : 4;
        ...
        private async void RaiseUpdate(IObject obj)
        {
            try
            {
                Task task = obj is UpdatesBase updates ? OnUpdate?.Invoke(updates) : OnOther?.Invoke(obj);
        ...
        private async Task DoConnectAsync(bool quickResume)
        {
            ...
            if (MTProxyUrl != null)
            {
                ...
                System.Collections.Specialized.NameValueCollection parms = HttpUtility.ParseQueryString(MTProxyUrl[MTProxyUrl.IndexOf('?')..]);
                string server = parms["server"];
                int port = int.Parse(parms["port"]);
                string str = parms["secret"]; // can be hex or base64
                byte[] secretBytes = secret = str.All("0123456789ABCDEFabcdef".Contains) ? Convert.FromHexString(str) :
                    System.Convert.FromBase64String(str.Replace('_', '/').Replace('-', '+') + new string('=', (2147483644 - str.Length) % 4));
                bool tlsMode = secret.Length >= 21 && secret[0] == 0xEE;
            ...
            else
            {
                ...
                try
                {
                    ...
                    catch (SocketException ex) // cannot connect to target endpoint, try to find an alternate
                    {
                        ...
                        HashSet<IPEndPoint> triedEndpoints = new() { endpoint };
                        if (_session.DcOptions != null)
                        {
                            IOrderedEnumerable<DcOption> altOptions = _session.DcOptions.Where(dco => dco.id == _dcSession.DataCenter.id && dco.flags != _dcSession.DataCenter.flags
                            ...
                            foreach (DcOption dcOption in altOptions)
                            {
            ...
            try
            {
                ...
                Task keepAliveTask = KeepAlive(_cts.Token);
                ...
                {
                    JSONValue initParams = JSONValue.FromJsonElement(System.Text.Json.JsonSerializer.Deserialize<System.Text.Json.JsonElement>(Config("init_params")));
        ...
        {
            _loginCfg.request = new();
            Func<string, string> prevConfig = _config;
            _config = what =>
            {
        ...
        public async Task<User> LoginBotIfNeeded(string bot_token = null)
        {
            ...
            if (_session.UserId != 0) // a user is already logged-in
            {
                try
                {
                    UserBase[] users = await this.Users_GetUsers(InputUser.Self); // this calls also reenable incoming Updates
                    User self = users[0] as User;
            ...
            Auth_AuthorizationBase authorization = await this.Auth_ImportBotAuthorization(0, _session.ApiId, _apiHash ??= Config("api_hash"), botToken);
        ...
        public async Task<User> LoginUserIfNeeded(CodeSettings settings = null, bool reloginOnFailedResume = true)
        {
            ...
            if (_session.UserId != 0) // a user is already logged-in
            {
                try
                {
                    UserBase[] users = await this.Users_GetUsers(InputUser.Self); // this call also reenable incoming Updates
                    User self = users[0] as User;
                    ...
                    string mismatch = $"Current logged user {self.id} mismatched user_id or phone_number";
...
#pragma warning disable CS0618 // Auth_* methods are marked as obsolete
            ...
            try
            {
                if (sentCodeBase is Auth_SentCode { type: Auth_SentCodeTypeSetUpEmailRequired setupEmail } setupSentCode)
                {
                    ...
                    {
                        EmailVerifyPurposeLoginSetup purpose = new() { phone_number = phone_number, phone_code_hash = phone_code_hash };
                        if (email is not "Google" and not "Apple")
                        {
                            Account_SentEmailCode sentEmail = await this.Account_SendVerifyEmailCode(purpose, email);
                        ...
                        for (int retry = 1; verified == null; retry++)
                            try
                            {
                                string code = await ConfigAsync("email_verification_code");
                                if (email is "Google")
            ...
            resent:
                ...
                else if (sentCodeBase is Auth_SentCode sentCode)
                {
                    ...
                    DateTime timeout = DateTime.UtcNow + TimeSpan.FromSeconds(sentCode.timeout);
                    Helpers.Log(3, $"A verification code has been sent via {sentCode.type.GetType().Name[17..]}");
                    RaiseUpdate(sentCode);
                    if (sentCode.type is Auth_SentCodeTypeFirebaseSms firebaseSms)
                    {
                        string token = await ConfigAsync("firebase");
                    ...
                    for (int retry = 1; authorization == null; retry++)
                        try
                        {
                            string verification_code = await ConfigAsync("verification_code");
                            if (verification_code == "" && sentCode.next_type != 0)
                            {
                                TimeSpan mustWait = timeout - DateTime.UtcNow;
                                if (mustWait.Ticks > 0)
                                {
                        ...
                        catch (RpcException e) when (e.Code == 401 && e.Message == "SESSION_PASSWORD_NEEDED")
                        {
                            for (int pwdRetry = 1; authorization == null; pwdRetry++)
                                try
                                {
                                    Account_Password accountPassword = await this.Account_GetPassword();
                                    RaiseUpdate(accountPassword);
                                    InputCheckPasswordSRP checkPasswordSRP = await Check2FA(accountPassword, () => ConfigAsync("password"));
            ...
            {
                DateTime waitUntil = DateTime.UtcNow.AddSeconds(3);
                RaiseUpdate(signUpRequired); // give caller the possibility to read and accept TOS
                string first_name = Config("first_name");
                string last_name = Config("last_name");
                TimeSpan wait = waitUntil - DateTime.UtcNow;
...
#pragma warning restore CS0618
        ...
        private MsgsAck CheckMsgsToAck()
        {
            lock (_msgsToAck)
            {
                ...
                MsgsAck msgsAck = new() { msg_ids = [.. _msgsToAck] };
                _msgsToAck.Clear();
                return msgsAck;
            }
        }
        ...
        private async Task SendAsync(IObject msg, bool isContent, Rpc rpc = null)
        {
            ...
            {
                (long ackId, int ackSeqno) = NewMsgId(false);
                MsgContainer container = new() { messages = [new(msgId, seqno, msg), new(ackId, ackSeqno, msgsAck)] };
            ...
            try
            {
                using MemoryStream memStream = new(1024);
                ...
                {
                    CheckSalt();
                    using MemoryStream clearStream = new(1024);
                    using BinaryWriter clearWriter = new(clearStream);
                    ...
                    byte[] msgKeyLarge = _sha256.ComputeHash(clearBuffer, 0, 32 + clearLength + padding);
                ...
                {
                    byte[] padding = new byte[_random.Next(_dcSession.AuthKeyID == 0 ? 257 : 16)];
        ...
        internal async Task<T> InvokeBare<T>(IMethod<T> request)
        {
            ...
            object result = await _bareRpc.Task;
        ...
        public async Task<T> Invoke<T>(IMethod<T> query)
        {
            ...
            bool got503 = false;
        retry:
            Rpc rpc = new() { type = typeof(T) };
            await SendAsync(query, true, rpc);
            object result = await rpc.Task;
            switch (result)
            {
